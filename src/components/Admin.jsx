...
export default function Admin() {
    ...
    if (checkingSession) {
        return <div className="flex min-h-screen items-center justify-center bg-cream text-ink">Chargement…</div>
    }
    ...
    return (
        <div className="flex min-h-screen items-center justify-center bg-cream px-6">
            <form onSubmit={handleSubmit} className="w-full max-w-sm rounded-2xl bg-white p-8 shadow-sm">
                <h1 className="font-display text-2xl font-extrabold uppercase text-ink">Connexion admin</h1>
                <p className="mt-1 text-sm text-ink/60">Gestionnaire produits Casa Beauté</p>
                ...
                <label className="mt-4 block text-xs uppercase tracking-wideish text-ink/50">Mot de passe</label>
                <input
                    type="password"
                    required
                    value={password}
    ...
    useEffect(() => {
        if (unseenCount > 0 && !titleIntervalRef.current) {
            ...
            titleIntervalRef.current = setInterval(() => {
                document.title = flash ? ORIGINAL_TITLE : `🔔 Nouvelle commande !`
                flash = !flash
            }, 1000)
        }
        if (unseenCount === 0 && titleIntervalRef.current) {
        ...
        <div className="min-h-screen bg-cream px-6 py-10 sm:px-10 lg:px-16">
            {banner && (
                <div className="fixed left-1/2 top-4 z-[200] w-[92%] max-w-sm -translate-x-1/2 rounded-2xl bg-ink px-5 py-4 text-cream shadow-xl">
                    <p className="font-display text-sm font-bold uppercase tracking-wideish">🔔 Nouvelle commande</p>
                    <p className="mt-1 text-sm text-cream/80">De {banner.customer_name} — {banner.phone}</p>
                    <button
                        onClick={goToOrders}
            ...
            <div className="mx-auto max-w-5xl">
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="mt-1 text-sm text-ink/60">Gérez vos produits et vos commandes.</p>
                    </div>
                    <button
        ...
        <div>
            ...
            <div className="mt-10 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                ...
                {!loading && products.length === 0 && (
                    <p className="text-ink/50">Aucun produit pour le moment — ajoutez le premier ci-dessus.</p>
                )}
                {products.map((p) => (
                    <div key={p.id} className="rounded-2xl bg-white p-4 shadow-sm">
                        ...
                        <label className="mt-3 flex cursor-pointer items-center gap-2">
                            ...
                            />
                            <span className="text-xs text-ink/60">Nouveauté</span>
                        </label>
...
function ProductForm({ product, onClose }) {
    ...
    return (
        <form onSubmit={handleSubmit} className="mt-6 rounded-2xl bg-white p-6 shadow-sm">
            <h2 className="font-display text-xl font-bold text-ink">{isEdit ? 'Modifier le produit' : 'Nouveau produit'}</h2>
            ...
            <label className="mt-4 block text-xs uppercase tracking-wideish text-ink/50">Catégorie</label>
            <select
                value={category}
            ...
            <label className="mt-4 block text-xs uppercase tracking-wideish text-ink/50">Prix</label>
            <input
                value={price}
                onChange={(e) => setPrice(e.target.value)}
                placeholder="ex. 2 400 DA"
                className="mt-1 w-full rounded-lg border border-ink/15 px-4 py-2.5 text-sm outline-none focus:border-rose-deep"
            />
            ...
            <label className="mt-4 block text-xs uppercase tracking-wideish text-ink/50">Quantité en stock</label>
            <input
                type="number"
                value={stock}
            ...
            <label className="mt-5 flex cursor-pointer items-center gap-3 rounded-xl border-2 border-rose-deep/30 bg-rose-deep/5 px-4 py-3 transition-colors hover:border-rose-deep/60">
                ...
                />
                <span>
                    <span className="block text-sm font-medium text-ink">✨ Marquer comme nouveauté</span>
                    <span className="block text-xs text-ink/50">
                        Affiche ce produit dans la section « Nouveautés » de la page d'accueil, sous sa catégorie.
                    </span>
                </span>
            </label>
...
function OrdersTab() {
    ...
    return (
        <div className="mt-8">
            {loading && <p className="text-ink/50">Chargement des commandes…</p>}
            {!loading && orders.length === 0 && <p className="text-ink/50">Aucune commande pour le moment.</p>}
